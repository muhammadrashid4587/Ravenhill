...
async def list_people(
    ...
    """
    # 1. Pull contacts. Falls back to seed data when Google isn't connected.
    ...
    # 2. Caller's email domain (if any) — drives the same-domain match.
    caller_email = (caller.email or "").strip().lower()
    caller_domain = _domain_of(caller_email)

    # 3. Pull all active agents with an email. We do this in one query and
    #    intersect in memory — N is small (one org, dozens to hundreds).
    ...
    # Stable order — domain colleagues first, then alphabetical.
