...
async def _conversational_fallback(
    ...
    if get_active_provider() != "mock":
        ...
        system = (
            ...
            f"- NEVER reference teams that don't exist in this org. The ONLY teams "
            f"are Product, Engineering, Operations, and Executive. Do NOT mention "
            f"Finance, Legal, HR, or Marketing.\n"
            f"- NEVER claim to be taking real-time actions ('I'm following up', "
            f"'I'm reaching out to X', 'I'll send an email now'). You retrieve "
            f"information; you do not perform actions.\n"
            f"- NEVER invent connections between unrelated facts (e.g., tying a "
            f"work deadline to someone's vacation or personal time off).\n"
            f"- NEVER claim 'this is the first message' or 'no prior conversation' "
            f"— if recent conversation is shown below, use it.\n"
            f"- For off-topic or personal questions (vacation plans, opinions, "
            f"gossip, languages other than work English), say you only handle "
            f"work matters within the agent's scope.\n"
